...
error-multiple-targets = 指定了多个目标。
error-v4-v6-conflict = -4 和 -6 不能同时使用。
error-value-range = 选项 -{ $flag } 的值必须介于 { $min } 和 { $max } 之间。
error-value-numeric = 选项 -{ $flag } 的值必须是数字。
