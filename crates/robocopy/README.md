...
| `/XD` 排除目录的 Dirs 统计 | 原版 `/XD` 排除的子目录仍计入 Dirs Total（访问但不处理）；本实现直接跳过不统计（阶段 2 遗留，待对齐） |
| `/MT` + `/V` 的 skipped 标签 | 中文 locale 下原版部分文件显示本地化标签（如"相同"），本实现统一英文（原版输出不稳定） |
| `/MT` 全局并行度 | 本实现按目录粒度批次并行（同目录多文件并行、跨目录串行），输出顺序稳定；原版全局线程池、输出乱序 |
